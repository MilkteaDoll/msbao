...
export function applyMapleCommands(ctx: Context, config: Config, api: MapleStoryApi, bindingsMgr: BindingsManager) {
    ...
    // 内部辅助函数：根据角色名获取基本信息
    async function getCharacterBasicInfo(name: string) {
        const character = await api.getCharacter(name);
        const ocid = character.ocid;
        if (!ocid) throw new Error('查询失败，请检查角色名');
        const basic = await api.getCharacterBasic(ocid);
        if (!basic) throw new Error('查询失败，请检查角色名');
        return basic;
    }

    // 内部辅助函数：获取角色经验变化数据
    async function getCharacterExpHistory(name: string) {
        const character = await api.getCharacter(name);
        const ocid = character.ocid;
        if (!ocid) throw new Error('查询失败，请检查角色名');
    ...
    ctx.command('%我的信息', '查询绑定的游戏角色信息')
        .action(async ({ session }) => {
            ...
            try {
                const basic = await getCharacterBasicInfo(boundGameId);
    ...
    ctx.command('%我的经验', '查看绑定角色最近7天经验变化')
        ...
        .action(async ({ session }) => {
            ...
            try {
                const lines = await getCharacterExpHistory(boundGameId);
    ...
    ctx.command('%查岗 <name:string>', '查看角色最近7天经验变化')
        ...
        .action(async ({ session }, name) => {
            ...
            try {
                const lines = await getCharacterExpHistory(name);
